...
namespace Certificates
{
    ...
    public class CreateCertificate
    {
        ...
        {
            X509Certificate2 certificate = CreateCert("CN=SelfSignedTestCert", RSA.Create(2048), HashAlgorithmName.SHA256, null);
            DisplayParameters(certificate, "SelfSignedTestCert");
        ...
        private static void DisplayParameters(X509Certificate2 x509Certificate2, string name)
        {
            ...
            Console.WriteLine($"public static X509Certificate2 {name}Private = new X509Certificate2(Convert.FromBase64String(\"{Convert.ToBase64String(x509Certificate2.Export(X509ContentType.Pfx, name))}\"), \"{name}\", X509KeyStorageFlags.EphemeralKeySet);");
            Console.WriteLine();
            Console.WriteLine($"public static X509Certificate2 {name}Public = new X509Certificate2(Convert.FromBase64String(\"{Convert.ToBase64String(x509Certificate2.Export(X509ContentType.Cert))}\"));");
            Console.WriteLine();
